...
public interface AddressDao extends CrudRepository<Address, Integer> {
	...
	@Query("SELECT new it.istat.mec.regedit.dto.ReportPivotDto(cast(adr.dataMod as date), SUM(CASE WHEN  adr.stato =  1 THEN 1 ELSE 0 END)   ,"
			...
			+ " AND ((:dateModSup is NULL) OR (adr.dataMod <=:dateModSup)) AND  ((:dateModInf is NULL) OR (adr.dataMod >= :dateModInf))"
			+ " GROUP BY cast(adr.dataMod as date)"
			+ " ORDER By 1 ASC ")
	List<ReportPivotDto> getReportTotalDailyPivotAddressStateUser(@Param("dateModInf") Date dateModInf,@Param("dateModSup") Date dateModSup);
}
